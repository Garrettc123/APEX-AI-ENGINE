"""Executor worker entrypoint — `python -m apex.workers.executor`.

Runs a Celery worker bound to the `executor` queue, consuming tasks from
`apex.workers.tasks.executor_tasks`. This wraps the existing Celery task
