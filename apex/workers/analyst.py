"""Analyst worker entrypoint — `python -m apex.workers.analyst`.

Runs a Celery worker bound to the `analyst` queue, consuming tasks from
`apex.workers.tasks.analyst_tasks`. This wraps the existing Celery task
